...
const Test = ({}) => {
	...
	const first_time_second = useSelector(state => state.user.first_time_second);
	...
	useEffect(() => {
		if (first_time && document.querySelector('.slick-current') != null && !isMobile) {
		...
		if (!first_time && first_time_second && !isMobile && document.querySelector('.slick-current') != null) {
			const el = document.querySelector('.slick-current').firstChild.firstChild.childNodes[2].childNodes[1];
		...
		if (!first_time && first_time_second && isMobile && document.querySelector('.brands-list__item') != null) {
			const el = document.querySelector('.brands-list__item').childNodes[0].childNodes[2].childNodes[1];
			const set_el = document.querySelector('img.helped-container__img_second');
	...
	return (
		<div className="page archive-page test-page ">
			<Header />
			{first_time && !isMobile ? (
				<div className="helped-container" onClick={() => dispatch({ type: SET_FIRST_TIME })}>
					<img className="helped-container__img" src={pkHelp} />
				</div>
			) : !first_time && first_time_second && !isMobile ? (
				<div className="helped-container" onClick={() => dispatch({ type: SET_FIRST_TIME_SECOND })}>
			...
			<div className="test-content">
				{slides.length > 0 && !isMobile && (
					<>
						<SamplePrevArrow onClick={() => swiperRef.current?.swiper.slidePrev()} />
						...
						<SampleNextArrow onClick={() => swiperRef.current?.swiper.slideNext()} />
					</>
				)}
			</div>
